...
| V-EMAIL-outbound | Outbound letter dispatch (Resend) | ✅ | `workers/api/src/intervention/send.ts` | pending |
...
- 2026-04-22: V-EXT-INLINE-i ✅ — Marketplace counterfeit inline badge LIVE. Five-host content-script module (eBay `/itm/`, Amazon 3P seller `/sp/` OR `?m=/smid=/seller=`, Facebook Marketplace `/marketplace/item/`, Walmart 3P `/seller/` OR `?sellerId=`, Mercari `/us/item/`) with per-host scraper table (primary + fallback selectors); reuses the S3-W18 `/counterfeit/check` 6-signal deterministic detector. Color-coded shadow-DOM badge near the price anchor: silent on authentic-with-zero-signals (Apple-bar §6), amber for caution/monitor, red for likely-counterfeit with top-signal headline + expandable signal list + risk-score chip. 25 tests (10 detectMarketplace + 6 scrapeListing + 2 priceAnchor + 4 renderBadge incl. silent-on-clean + double-render guard, 2 deriveCategory + 2 postCounterfeitCheck PII-regression). Judge P0/P1 applied in-block: (P0-1) `marketplace` key stripped from POST body — backend `CounterfeitRequestSchema` is `.strict()` and would 400 otherwise; (P0-2) require `snapshot.price !== undefined` before POSTing so insufficient-data-only results don't paint amber on every listing; (P0-3) derive coarse category from productName with 9-keyword match table (espresso-machine/laptop/smartphone/headphones/watch/camera/tv/sneakers/handbag) so the price-too-low fail signal can actually fire; (P0-4) fixed FB Marketplace price scrape — scan currency-shaped spans in reading order instead of regex-over-2000-char-blob that catches "$5 shipping"; (P0-5) surface Walmart `sellerId` from URL param; (P1-6) `retailReboot` sweeps stale `[data-lens="counterfeit-host"]` nodes + clears `data-lens-counterfeit` anchor attrs on SPA nav so eBay pushState churn doesn't leave dead badges; (P1-7) gate Amazon histogram scrape on `/sp/` path only (was incorrectly scraping product-review stars as seller-feedback bimodality on `/dp/`); (P1-9) regression test that POST body has NO `sellerName` or `sellerId` keys (PII never on wire per AMBIENT_MODEL §2 Stage-2 contract); (P3-12) `priceAnchor("fb-marketplace")` now prefers a currency-shaped span near the title. Rate-limit policy `counterfeit-check` added (120/hr anon, 1200/hr user; eBay pushState churn). Manifest host_permissions + content_scripts matches extended to ebay.com, facebook.com/marketplace, mercari.com. Deployed worker version `dad4b523-e4e5-4cf9-a751-d72ae4dc3639`. Commit `pending`.
- 2026-04-22: CJ-W53 calibration fix (user-feedback regression) ✅ — Pinned in same commit as V-EXT-INLINE-i. User paste-tested with Anker foldable-charging-stand URL; chat (a) kept using em-dashes despite greeting fix, (b) assumed a preference from the user's question "why are you asking about stationary desk stand" instead of answering it, (c) triggered the audit on user's 4th turn which was itself a question. Fixes: STAGE1_ELICIT_SYSTEM + STAGE3_RECOMMEND + STAGE4_FOLLOWUP now carry explicit "NEVER use em-dashes" rule + Stage-1 "if the user's last message is a question to you, ANSWER IT first" branch + emoji budget tightened 2 → 0-1. `scrubClarifierText` normalizes any residual em/en-dash to ", ". `isReadyToGenerate` + `shouldTriggerAudit` both early-return false when `lastUserEndedInQuestion` — detects trailing `?` / `？` / `؟` AND question-word openers (what/why/how/which/can you/could you/would you) without punctuation. 2 new tests lock the behavior (plain preference at 4 user turns still triggers; question at 4 user turns does NOT).
- 2026-04-22: CJ-W53 ✅ — Conversational preference elicitor (chat-first front door) LIVE. Replaces the paste-box + streaming dev-log UI with a Study-3 ecological-validity chatbot pattern (Affonso et al., Nature-submitted 2026, READ-ONLY reference in `projects/spec-resistance/nature-rr/study3-chatbot/`). Backend: POST /chat/clarify (Opus 4.7 drafts next clarifier question OR returns READY token + per-category fallback bank + 20-pattern affiliate scrub) + POST /chat/followup (1M-context Opus answers follow-up questions over full audit result; never re-audits). Stop-gate ports Study 3's `userTurns >= 4 || (userTurns >= 3 && !lastBotEndedInQ)` verbatim + userGaveEverything fast-path shortcut (budget regex + tradeoff keyword match). Web: ChatView.ts orchestrator (greeting → clarify loop → audit wall with 5-phrase rotator that holds on final phrase → recap bubble → existing audit card drops underneath → follow-up loop); ConversationStore with localStorage persistence + 50-turn cap; stop-mirror front-end for responsive UI; bubbleRenderer with safe bold-only escape; composer with Enter-to-send + auto-grow; rotatingStatus with compose-focus pause (P1-5). Chat is default-ON; ?chat=0 disables + persists to localStorage + surfaces a "Switch to classic" toggle in the footer. 30 worker tests (stops 13 + clarify 11 + followup 6) + 15 web tests (stages 8 + ConversationStore 7). All green. Rate-limits: chat-clarify 60/hr anon 600/hr user, chat-followup 40/hr anon 400/hr user. Judge P0/P1 applied in-block: em-dash out of greeting, audit-prompt now folds assistant-clarifier-Q + user-A pairs, new-topic detector routes back to Stage-1 instead of followup-against-stale-audit, affiliate scrub expanded from 10 → 20+ patterns, non-ASCII question-mark support, rotator pauses on composer focus, reduced-motion full coverage, focus rings on chips + send button, running-earbuds + blender fallbacks updated to 2026 reality, "Start a new shopping question" chip renders after card drops. Deployed: worker version `02891bd5-2a71-4faf-9bcd-b1136e3020ad` + web Pages. Commit `f9637c9` (follows initial `fec08a2`). Live smoke: /chat/clarify returns "Hey! ☕ What's your budget... **fully automatic** or **semi-automatic**?" on espresso query, `{kind:"ready",source:"stop-logic"}` on satiated input.
- 2026-04-22: V-EXT-INLINE-h ✅ — Amazon review-authenticity inline banner LIVE. Scrapes `[data-hook="review"]` markup on product pages + `/product-reviews/` pages; POSTs `/review-scan` (existing W17 heuristic: temporal clustering, language homogeneity, rating skew, template phrasing, length homogeneity); renders shadow-DOM banner above the reviews anchor with color-coded band (green ≥0.7 clean / amber 0.4-0.7 mixed / red <0.4 suspect). Headline adapts to band: suspect shows "Likely incentivized — N of M reviews suspect"; mixed shows "N of M flagged" or "Mixed review signals (pct%)"; clean shows "Reviews look authentic (pct%)". Silent-unless-signal: clean + zero signals renders nothing. Consent gate via canStage2() (review text IS Stage-2 excerpt traffic per AMBIENT_MODEL §2). 16 tests covering URL context detection, scrape selectors (data-hook wrappers, body, rating, date, reviewer), anchor fallback chain (cm-cr-dp-review-list → cm_cr-review_list → reviews-medley-footer → customerReviews), renderBanner silent-on-clean + double-render guard. Judge P0/P1 applied in-block: (P0-1) reviewer PII stripped client-side before POST; (P0-2) single-flight `inFlight` boolean blocks duplicate fetches during setTimeout-boot + retailReboot pushState race; (P0-3) per-ASIN 10-min cache — Amazon's sort/filter/paginate pushStates inside /product-reviews/ no longer each burn rate-limit budget; (P1-4) UK/day-first date parser ("3 March 2025") for amazon.co.uk temporal-clustering; (P1-5) .caveat opacity raised 0.7 → 0.85 (AA contrast on amber + red bands); (P1-6) role="status" moved off wrapping `<section>` onto sr-only sibling span so NVDA doesn't double-announce when button is nested inside a live region; (P3-10) pct/100 hidden on suspect band (redundant with headline). Also bundled UX polish: humanizeCriterion() helper converts snake_case (`cpu_performance` → "CPU performance") in criteriaCard + claimRow, conversational pipeline status ("Looking at 12 real products" not "search:done · 12"), friendlier stream header ("Lens is on it…"). Deployed https://532875e8.lens-b1h.pages.dev. Commit `a15a355`.
- 2026-04-22: V-EXT-INLINE-f ✅ — cart-page inline checkout-readiness badge LIVE. Extension detects /cart, /checkout, /basket, /booking/confirm, /payment URL patterns; reuses passive-scan hit list; POSTs /checkout/summary with `{host, signals:{passiveScan:{confirmedCount, topPattern?, ran}}}`; renders shadow-DOM proceed/hesitate/rethink badge with expandable top-3 rationale list near the retailer's subtotal anchor (6 per-host selectors). Silent-unless-signal: `proceed + signalCount=0` → no badge. 10 tests covering URL detection + 6 per-host anchors. Judge P0/P1 applied in-block: (P0-1) spec-vs-code gap — code supports 6 retailers only (detectHost), so /booking/confirm + /payment don't render even though URL detection accepts them (documented in block file); (P0-2) new /checkout/summary rate-limit policy (120/hr anon, 1200/hr user); (P0-3) SPA reattach via popstate + pushState monkey-patch (shared with V-EXT-INLINE-g — bothretail boots now re-run); (P1-1) document-wide guard on `[data-lens="cart-summary-host"]` prevents double-badge when SPA re-render wipes anchor state; (P1-2) anchor scoping tightened from `anchor.closest("section, aside, [role=region]")` to `anchor.parentElement` so the badge stays visually adjacent to the subtotal. P1-3 (fetch-path tests), P1-4 (consent-coupling docs), P2/P3 (a11y role cleanup, hardcoded "heuristic-only") tracked for V-EXT-INLINE-polish block. Commit `b7215c2`.
