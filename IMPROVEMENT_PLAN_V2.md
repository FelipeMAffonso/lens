...
- 2026-04-23 ~06:10 EDT: **VISION #31** ✅ (F2) — OpenAPI 3.1 spec + interactive docs page. `workers/api/src/openapi/spec.ts` enumerates 28 public routes (core, auth, audit, sku, triggers, shopping-session, visual, embed, push, digest, ticker); `workers/api/src/openapi/docs.ts` renders Scalar API Reference over CDN with warm-bone palette + Source Serif 4 header. Served at `/openapi.json` (cache 5 min) and `/docs`. Landing-page nav gains "API docs ↗" link. Deploy verified: `/openapi.json` returns 200 with 28 paths; `/docs` returns HTML with the header scaffold intact. Worker uploaded 84.3s.
